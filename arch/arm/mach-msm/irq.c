...
static void do_msm_irq_ack(unsigned int irq)
{
...
static int do_msm_irq_set_type(unsigned int irq, unsigned int flow_type)
{
...
{
	return do_msm_irq_set_type(d->irq, flow_type);
}
...
int msm_irq_enter_sleep2(bool arm9_wake, int from_idle)
{
	...
	if (arm9_wake) {
		do_msm_irq_set_type(INT_A9_M2A_6, IRQF_TRIGGER_RISING);
		do_msm_irq_ack(INT_A9_M2A_6);
...
void msm_fiq_enable(int irq)
{
	...
	irq_desc_get_chip(irq_to_desc(irq))->irq_unmask(irq_get_irq_data(irq));
...
void msm_fiq_disable(int irq)
{
	unsigned long flags;
	local_irq_save(flags);
	irq_desc_get_chip(irq_to_desc(irq))->irq_mask(irq_get_irq_data(irq));
	local_irq_restore(flags);
}
