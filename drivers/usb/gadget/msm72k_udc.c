...
static void dock_isr_work(struct work_struct *w)
{
	...
	if (ui->dock_pin_state == 1)
		irq_set_irq_type(ui->dockpin_irq, IRQF_TRIGGER_LOW);
	else
		irq_set_irq_type(ui->dockpin_irq, IRQF_TRIGGER_HIGH);
...
static void dock_detect_init(struct usb_info *ui)
{
	...
	if (!vbus)
		irq_set_status_flags(ui->dockpin_irq, IRQF_VALID | IRQF_NOAUTOEN);
	...
	ret = irq_set_irq_wake(ui->dockpin_irq, 1);
	if (ret < 0) {
...
static void accessory_detect_work(struct work_struct *w)
{
	...
	value = gpio_get_value(ui->usb_id_pin_gpio);
	if (value == 0)
		irq_set_irq_type(ui->idpin_irq, IRQF_TRIGGER_HIGH);
	else
		irq_set_irq_type(ui->idpin_irq, IRQF_TRIGGER_LOW);
...
static void accessory_detect_init(struct usb_info *ui)
{
	...
	ret = irq_set_irq_wake(ui->idpin_irq, 1);
	if (ret < 0) {
			...
			if (vbus)
				...
				if (cancel_delayed_work_sync(&ui->dock_work)) {
					if (ui->dock_pin_state == 0)
						irq_set_irq_type(ui->dockpin_irq,
							IRQF_TRIGGER_LOW);
				}
				if (ui->accessory_type == 3) {
