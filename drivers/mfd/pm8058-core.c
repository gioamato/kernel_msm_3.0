...
static void pm8058_irq_ack(struct irq_data *d)
{
	struct pm8058 *pmic = irq_get_chip_data(d->irq);
	unsigned int irq = d->irq - pmic->irq_base;
	unsigned long flags;
...
static void pm8058_irq_mask(struct irq_data *d)
{
	struct pm8058 *pmic = irq_get_chip_data(d->irq);
	unsigned int irq = d->irq - pmic->irq_base;
...
static void pm8058_irq_unmask(struct irq_data *d)
{
	struct pm8058 *pmic = irq_get_chip_data(d->irq);
	unsigned int irq = d->irq - pmic->irq_base;
	struct pm8058_irq_info *irq_info = &pmic->irqs[irq];
...
static void pm8058_irq_disable(struct irq_data *d)
{
	//struct irq_desc *desc = irq_to_desc(d->irq);

	pm8058_irq_mask(d);
	//desc->status |= IRQD_IRQ_MASKED;
}

static void pm8058_irq_enable(struct irq_data *d)
{
	//struct irq_desc *desc = irq_to_desc(d->irq);
...
static int pm8058_irq_set_type(struct irq_data *d, unsigned int flow_type)
{
	struct pm8058 *pmic = irq_get_chip_data(d->irq);
	unsigned int irq = d->irq - pmic->irq_base;
	struct pm8058_irq_info *irq_info = &pmic->irqs[irq];
...
static int pm8058_irq_set_wake(struct irq_data *d, unsigned int on)
{
	struct pm8058 *pmic = irq_get_chip_data(d->irq);
	unsigned int irq = d->irq - pmic->irq_base;
	struct pm8058_irq_info *irq_info = &pmic->irqs[irq];
...
static struct irq_chip pm8058_irq_chip = {
	...
	.irq_disable	= pm8058_irq_disable,
	.irq_enable	= pm8058_irq_enable,
...
static int pm8058_irq_init(struct pm8058 *pmic, unsigned int irq_base)
{
	...
	for (i = 0; i < NUM_IRQ_BANKS; ++i) {
		...
		for (j = 0; j < bank->cnt; ++j) {
			...
			irq_set_chip(irq_base + irq, &pm8058_irq_chip);
			irq_set_chip_data(irq_base + irq, pmic);
			irq_set_handler(irq_base + irq, handle_edge_irq);
...
static int pm8058_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		irq_set_handler_data(devirq, (void *)pmic);
